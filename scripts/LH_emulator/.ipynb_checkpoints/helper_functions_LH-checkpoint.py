...
def load_profiles_CMASS(usecols,home,suite,suiteset,sims,prof):
    y=[]
    for s in np.arange(len(sims)):
        f=f"{home}/LH_set/{suite}/{suite}_{suiteset}_{s}_024_w.txt"
        x,yi=np.loadtxt(f,usecols=usecols,unpack=True)
        yi=cgs_units(prof,yi)
        y.append(np.log10(yi))
...
def build_emulator_CMASS(home,suite,suiteset,prof,func_str):
    z=choose_redshift(suite)
    z=np.array([z[-1]])
    ...
    x,y=load_profiles_CMASS(usecols,home,suite,suiteset,sims,prof)
    y=np.transpose(y)
    emulator=ostrich.emulate.PcaEmulator.create_from_data(
        samples,
        y,
